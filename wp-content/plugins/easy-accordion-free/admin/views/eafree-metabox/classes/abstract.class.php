	...
	abstract class SP_EAP_Abstract {
		...
		public function add_enqueue_google_fonts() {
			...
			if ( ! empty( $this->pre_fields ) ) {
				...
				foreach ( $this->pre_fields as $field ) {
					...
					if ( $field_type && $field_id ) {
						...
						SP_EAP::maybe_include_field( $field_type );

						$class_name = 'SP_EAP_Field_' . $field_type;
			...
			if ( ! empty( $this->typographies ) && empty( $this->args['async_webfont'] ) ) {
				...
				$handle = 'SP_EAP-google-web-fonts-' . $this->unique;
		...
		public function add_output_css() {
			...
			$this->output_css = apply_filters( "SP_EAP_{$this->unique}_output_css", $this->output_css, $this );
