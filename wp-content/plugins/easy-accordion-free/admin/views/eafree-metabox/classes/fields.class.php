		...
		public function field_data( $type = '' ) {
			...
			switch ( $type ) {
				...
				case 'taxonomy':
					...
					if ( isset( $saved_meta['eap_post_type'] ) && ! empty( $saved_meta['eap_post_type'] ) ) {
						$taxonomy_names = get_object_taxonomies( $saved_meta['eap_post_type'], 'names' );
				...
				case 'term':
					global $post;
					$saved_meta = get_post_meta( $post->ID, 'sp_eap_upload_options', true );
					if ( is_object( isset( $saved_meta['eap_post_taxonomy'] ) ) && '' !== $saved_meta['eap_post_taxonomy'] ) {
